...
export default function KursyStudent() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (profile?.role !== 'STUDENT') {
        alert('Dostęp tylko dla uczniów');
        return navigate('/');
      }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      <Sidebar user={user} role="STUDENT" onLogout={handleLogout} />
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {courses.map(course => (
                <button
                  key={course.id}
                  onClick={() => navigate(`/student/kursy/${course.id}`)}
